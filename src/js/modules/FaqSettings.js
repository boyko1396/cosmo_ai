export default class FaqSettings {
  constructor() {
    this.faqCardBtnToggle = document.querySelectorAll('.js-faq-settings-toggle');
    this.setupEventListeners();
    this.activeCard = null;
  }

  toggleFaqCard(event) {
    const listItem = event.target.closest('.faq-settings');
  ...
  animateDropdown(listItem) {
    const dropdown = listItem.querySelector('.faq-settings__dropdown');
  ...
  updateDropdownHeight() {
    document.querySelectorAll('.faq-settings.is-show').forEach((card) => {
      const dropdown = card.querySelector('.faq-settings__dropdown');
      const height = dropdown.scrollHeight;
      dropdown.style.height = height + 'px';
    });
  }
